                ...
                @auth
                    @if ($post->checkLikes(auth()->user()))
                        <form action="{{ route('posts.likes.destroy', $post) }}" method="POST">
                            ...
                            <div class="my-4">
                                <button type="submit">
                                    <svg xmlns="http://www.w3.org/2000/svg" fill="red" viewBox="0 0 24 24" strokeWidth="2"
                                        class="h-6 w-6" stroke="currentColor" className="w-6 h-6">
                    ...
                    @if ($post->comentarios->count())
                        @foreach ($post->comentarios as $comentario)
                            <div class="p-5 border-gray-300 border-b">
                                <a href="{{ route('posts.muro', $comentario->user) }}" class="font-bold">
